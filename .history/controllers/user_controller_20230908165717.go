...
// update patient in patient collection and user in user collection at the same time
...
func UpdatePatient(c echo.Context) error {
	...
	if err := c.Bind(&patient); err != nil {
		return c.JSON(http.StatusBadRequest, responses.UserResponse{Status: http.StatusBadRequest, Message: "error in patient binding", Data: &echo.Map{"data": err.Error()}})
	}

	if err := c.Bind(&user); err != nil {
		return c.JSON(http.StatusBadRequest, responses.UserResponse{Status: http.StatusBadRequest, Message: "error in user binding", Data: &echo.Map{"data": err.Error()}})
	}

	if err := validate.Struct(patient); err != nil {
		return c.JSON(http.StatusBadRequest, responses.UserResponse{Status: http.StatusBadRequest, Message: "error in patient validation", Data: &echo.Map{"data": err.Error()}})
	}

	if err := validate.Struct(user); err != nil {
		return c.JSON(http.StatusBadRequest, responses.UserResponse{Status: http.StatusBadRequest, Message: "error in user validation", Data: &echo.Map{"data": err.Error()}})
	}

	_, err = patientCollection.UpdateOne(ctx, bson.M{"_id": objId}, bson.M{"$set": patient})

	if err != nil {
		return c.JSON(http.StatusInternalServerError, responses.UserResponse{Status: http.StatusInternalServerError, Message: fmt.Sprintf(" error in patient %s", objId), Data: &echo.Map{"data": err.Error()}})
	}

	_, err = userCollection.UpdateOne(ctx, bson.M{"_id": objId}, bson.M{"$set": user})

	if err != nil {
		return c.JSON(http.StatusInternalServerError, responses.UserResponse{Status: http.StatusInternalServerError, Message: fmt.Sprintf(" error in user %s", objId), Data: &echo.Map{"data": err.Error()}})
	}

	return c.JSON(http.StatusOK, responses.UserResponse{Status: http.StatusOK, Message: "success", Data: &echo.Map{"data": patient}})
}
