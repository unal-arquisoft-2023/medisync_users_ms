...
func GetPatient(c echo.Context) error {
	...
	defer cancel()
	err := patientCollection.FindOne(ctx, bson.M{"userid": "64fa35787e8079fa4c5115e7"}).Decode(&patient)

	// err := patientCollection.FindOne(ctx, bson.M{"userid": userId}).Decode(&patient)
	if err != nil {
		return c.JSON(http.StatusInternalServerError, responses.UserResponse{Status: http.StatusInternalServerError, Message: fmt.Sprintf(" error %s", userId), Data: &echo.Map{"data": err.Error()}})
	}
