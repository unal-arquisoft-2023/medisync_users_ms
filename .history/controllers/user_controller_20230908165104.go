...
func UpdatePatient(c echo.Context) error {
	...
	if err := c.Bind(&patient); err != nil {
		return c.JSON(http.StatusBadRequest, responses.UserResponse{Status: http.StatusBadRequest, Message: "error in patient", Data: &echo.Map{"data": err.Error()}})
	}

	if err := c.Bind(&user); err != nil {
		return c.JSON(http.StatusBadRequest, responses.UserResponse{Status: http.StatusBadRequest, Message: "error in user", Data: &echo.Map{"data": err.Error()}})
	}
