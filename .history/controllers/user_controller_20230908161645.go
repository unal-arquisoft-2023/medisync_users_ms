...
// get all info of a patient, including user info, using createPatientRequest struct
func GetPatient(c echo.Context) error {
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	userId := c.Param("userId")

	var patient dtos.CreatePatientRequest
	defer cancel()

	objId, _ := primitive.ObjectIDFromHex(userId)
	err := userCollection.FindOne(ctx, bson.M{"id": objId}).Decode(&patient)
